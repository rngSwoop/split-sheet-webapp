...
export async function POST() {
  try {
    ...
    for (const user of usersWithoutUsername) {
      ...
      // Check if username field exists in database before trying to use it
      try {
        await prisma.user.findFirst({
          where: { username: 'test-check' }
        });
      } catch (err) {
        // Username field not available yet, skip updates
        continue;
      }
      ...
      while (attempts < maxAttempts) {
        try {
          ...
          // Username field not available yet
          break;
        }
      }
      ...
      try {
        ...
        // Skip if username field not available
        continue;
      }
    ...
    return NextResponse.json({ 
      message: `Generated usernames for ${updatedUsers.length} users`,
